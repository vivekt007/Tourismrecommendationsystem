...
def multiple_choice_question(request):
    options = {
        'water': [
            {'question': 'Which one do you prefer?', 'choices': ['Theme park', 'Beach']},
            {'question': 'What weather do you prefer?', 'choices': ['20c - 24c', '25c - 27c', '28c - 30c']},
            {'question': 'How many people will be travelling?', 'choices': ['1-2', '3-6', '7-10']},
        ],
        'snow': [
            {'question': 'Do you prefer skiing or snowboarding?', 'choices': ['Skiing', 'Snowboarding']},
            {'question': 'What weather do you prefer?', 'choices': ['20c - 24c', '25c - 27c', '28c - 30c']},
            {'question': 'How many people will be travelling?', 'choices': ['1-2', '3-6', '7-10']},
        ],
        'trekking': [
            {'question': 'How long of a hike do you prefer?', 'choices': ['Short (less than 2 hours)', 'Medium (2-4 hours)', 'Long (more than 4 hours)']},
            {'question': 'What weather do you prefer?', 'choices': ['20c - 24c', '25c - 27c', '28c - 30c']},
            {'question': 'How many people will be travelling?', 'choices': ['1-2', '3-6', '7-10']},
        ],
        'historical place': [
            {'question': 'Which period of history interests you the most?', 'choices': ['Ancient', 'Medieval', 'Modern']},
            {'question': 'What weather do you prefer?', 'choices': ['20c - 24c', '25c - 27c', '28c - 30c']},
            {'question': 'How many people will be travelling?', 'choices': ['1-2', '3-6', '7-10']},
        ],
        'adventure sports': [
            {'question': 'Which one do you prefer?', 'choices': ['Bungee jumping', 'Skydiving', 'Rock climbing']},
            {'question': 'What weather do you prefer?', 'choices': ['20c - 24c', '25c - 27c', '28c - 30c']},
            {'question': 'How many people will be travelling?', 'choices': ['1-2', '3-6', '7-10']},
        ],
    }
    if request.method == 'POST':
        ...
        additional_questions = options.get(answer, [])
        context = {
            'question1': answer, 
            'options': additional_questions
        }
    else:
        question = "Where do you feel like going?"
        options = ["Water", "Snow", "Trekking", "Historical place", "Adventure sports"]
        context = {
            'question': question,
            'options': options
        }

    return render(request, 'home2/multiple_choice_question.html', context)


def get_travel_questions(request):
    options = {
        'water': [
            {'question': 'Which one do you prefer?', 'choices': ['Theme park', 'Beach']},
            {'question': 'What weather do you prefer?', 'choices': ['20c - 24c', '25c - 27c', '28c - 30c']},
    ...
    if request.method == 'POST':
        ...
        additional_questions = options.get(question1, [])
        context = {'question1': question1, 'additional_questions': additional_questions}
        return render(request, 'home2/questionnaire.html', context)

    context = {'additional_questions': []}
    return render(request, 'home2/questionnaire.html', context)
...
def travel_questions(request):
    if request.method == 'POST':
        ...
        if question1 == 'water' and question2 == '20c - 24c':
            destinations = [
                {
                    'name': 'Pangong Lake',
                    'description': 'Pangong Tso, also referred to as Pangong Lake, is an enchanting lake situated in the Himalayas and stretches out from India to Tibet. The crystal-clear blue water and the surrounding snow-capped mountains make it one of the most scenic lakes in the world. You can enjoy camping, stargazing, and take a dip in the lake (if you can bear the cold water!).',
                    'image': 'https://example.com/images/pangong-lake.jpg'
                },
                {
                    'name': 'Tso Moriri Lake',
                    'description': 'Tso Moriri is a high-altitude lake situated in the Changthang region of Ladakh. It is surrounded by snow-capped mountains, and the turquoise blue water adds to the breathtaking scenery. You can spot several migratory birds and wildlife around the lake. It is an ideal location for camping, photography, and trekking.',
                    'image': 'https://example.com/images/tso-moriri-lake.jpg'
                }
            ]
        elif question1 == 'water' and question2 == '25c - 27c':
            destinations = [
                {
                    'name': 'Gokarna',
                    'description': 'For an unconventional beach destination, visit Gokarna in the Karnataka region. Gaining popularity over the years for its seclusion and pristine shores, this is one place you have to visit before tourism takes over.',
                    'image': 'https://static-blog.treebo.com/wp-content/uploads/2018/07/Gokarna-1.jpg'
                },
                {
                    'name': 'Tso Moriri Lake',
                    'description': 'Tso Moriri is a high-altitude lake situated in the Changthang region of Ladakh. It is surrounded by snow-capped mountains, and the turquoise blue water adds to the breathtaking scenery. You can spot several migratory birds and wildlife around the lake. It is an ideal location for camping, photography, and trekking.',
    ...
    return render(request, 'home2/questionnaire.html')

from django.shortcuts import render

def mcq2(request):
    if request.method == 'POST':
        # Get user's choices from the form
        location = request.POST.get('location')
        group_size = request.POST.get('group_size')
        temperature = request.POST.get('temperature')

        # Determine the holiday destination based on the user's choices
        destination = determine_destination(request)

        # Render the result template with the holiday destination
        return render(request, 'home2/result.html', {'destination': destination})
    else:
        # Render the MCQ template
        return render(request, 'home2/result.html')

    

from django.http import HttpResponse
# def determine_destination(request):
#     if request.method == 'POST':
...
#         # Create dictionary of destinations and their attributes
#         destinations = {
#             'Pangong Lake': {'location_type': 'historical', 'group_size': 1, 'temperature': (21,25)},
#             'Tso Moriri Lake': {'location_type': 'water', 'group_size': 5, 'temperature': 35},
#             'Mahabaleshwar': {'location_type': 'water', 'group_size': 2, 'temperature': 28},
#             'Gokarna': {'location_type': 'historical', 'group_size': 3, 'temperature': 20},
#             'Goa': {'location_type': 'city', 'group_size': 4, 'temperature': 18},
#             'Udaipur': {'location_type': 'historical', 'group_size': 2, 'temperature': 22},
#             'Kolkata': {'location_type': 'beach', 'group_size': 6, 'temperature': 30},
#         }

#         # Filter destinations based on user preferences
#         filtered_destinations = [k for k, v in destinations.items() if
#                                  v['location_type'] == location_type and
#                                  v['group_size'] >= group_size and
#                                  abs(v['temperature'] - temperature) <= 5]

#         if filtered_destinations:
#             # Return first destination in filtered list
#             return HttpResponse(f"We recommend {filtered_destinations[0]} as your holiday destination!")
#         else:
#             return HttpResponse("Sorry, we couldn't find a suitable destination for your preferences.")
#     else:
#         # Render the form
#         return render(request, 'home2/destination_form.html')
...
# def main(request):
#     if request.method == 'POST':
#         location_type = request.POST.get('mood')
#         group_size = request.POST.get('group_size')
#         temperature = request.POST.get('temp')

#         # Do something with the form data, such as print it
#         print('Location type:', location_type)
#         print('Group size:', group_size)
#         print('Temperature:', temperature)

#         # Return an HTTP response with the data
#         return HttpResponse(f"Location type: {location_type}, Group size: {group_size}, Temperature: {temperature}")
        
#     # If request method is GET, render the form template
#     return render(request, 'destination_form.html')
dataset = {
    'Andhra Pradesh': [
        {'place': 'Charminar', 'type': 'Historical','details':'Charminar - It is a historical monument and mosque located in the heart of Hyderabad. Built in 1591 by the fifth ruler of the Qutb Shahi dynasty, Mohammed Quli Qutb Shah, it is a popular tourist attraction and one of the most recognized landmarks of Hyderabad.' ,'url':'https://www.wonderwardrobes.com/wp-content/uploads/2015/06/Charminar-898x1024.jpg'},
        {'place': 'Tirumala Venkateswara Temple', 'type': 'Religious','details':'Tirumala Venkateswara Temple - It is a world-famous Hindu temple located on the hills of Tirumala in the state of Andhra Pradesh, about 20 km from the city of Tirupati in Chittoor district. It is one of the richest and most visited temples in India.' ,'url':'https://th.bing.com/th/id/OIP.1NEN7YnH9hztWWLzujbPGgHaEA?pid=ImgDet&rs=1'},
        {'place': 'Golconda Fort', 'type': 'Historical','details':'Golconda Fort - It is a historical fort located in Hyderabad. The fort was originally built in the 13th century by the Kakatiya dynasty and was later expanded by the Qutb Shahi dynasty. The fort is famous for its acoustics and its unique design, which allows sound to be heard from a distance of almost a kilometer.' ,'url':'https://th.bing.com/th/id/R.67efcb7734d2bb4f7f2401ac87868276?rik=uU%2fOZQyXe5wdtg&riu=http%3a%2f%2fwww.thesoultrails.com%2fwp-content%2fuploads%2f2018%2f11%2fGolconda-Fort-Hyderabad-Monuments-Heritage-3.jpg&ehk=Bh6YeDuKuhsp7poFXTT%2biB%2f8e3cmcl08%2bVey9wj7C%2f4%3d&risl=&pid=ImgRaw&r=0'},
        {'place': 'Araku Valley', 'type': 'Hill Station','details':'Araku Valley - It is a hill station located in the Visakhapatnam district of Andhra Pradesh. It is famous for its scenic beauty and is surrounded by lush green forests, waterfalls, and coffee plantations. The valley is also home to various tribes and is known for its rich cultural heritage.' ,'url':'https://th.bing.com/th/id/OIP.wK8N06IlwV2n2Gy-6b2brwAAAA?pid=ImgDet&rs=1'},
    ...
    'West Bengal': [
        ...
        {'place': 'Howrah Bridge', 'temp':26 ,'type': 'Historical','details':' It is an iconic suspension bridge that spans the Hooghly River in Kolkata. It was built in 1943 and is one of the busiest bridges in the world.' ,'url':'https://procaffenation.com/wp-content/uploads/2017/04/beautiful-view-of-howrah-bridge-at-evening-compressor.jpg'},
        {'place': 'Bishnupur', 'temp':24 ,'type': 'Historical','details':'Bishnupur: It is a small town in the Bankura district of West Bengal. It is known for its terracotta temples built during the 17th and 18th centuries by the Malla kings.' ,'url':'https://static.toiimg.com/photo/68741292/bishnupur-temples.jpg?width=748&resize=4'},
        ],
    }
...
#         location_type = request.POST.get('mood')
#         group_size = request.POST.get('group_size')
#         # Search for the details of the location based on user input
#         for state, locations in dataset.items():
#             for location in locations:
#                 if location_type.lower() == location['type'].lower():
#                     # Return the details of the location as context to the HTML template
#                     context = {
...
#                         'url': location['url']
#                     }
#                     return render(request, 'home2/destination_details.html', context)

#         # Return an HTTP response with an error message if no location is found
#         return HttpResponse("Sorry, we couldn't find any location that matches your criteria.")
...
##################################################
# def main(request):
#     # Get all the available location types
#     location_types = set()
#     for state, locations in dataset.items():
#         for location in locations:
#             location_types.add(location['type'])
...
#     # If request method is GET, render the form template
#     context = {'location_types': sorted(list(location_types))}
#     return render(request, 'home2/destination_form.html', context)
...
def main(request):
    ...
    # Set location_type to None by default
    location_type = None
    ...
    # Add an extra loop to iterate through all states and append locations with same type to context
    for state, locations in dataset.items():
        state_locations = []
        for location in locations:
            if location_type and location['type'].lower() == location_type.lower():
                # Add the matching location to the state_locations list
                state_locations.append(location)
        if state_locations:
            context[state] = state_locations
...
def travel_input(request):
    if request.method == 'POST':
        ...
        return HttpResponse("You entered the following details: <br>Location: {}<br>Start Date: {}<br>End Date: {}<br>Number of Days: {}<br>Number of People: {}<br>Companions: {}".format(location, start_date_str, end_date_str, num_days, num_people, companions))
    else:
        return render(request, 'home2/travel_input.html')
...
#             context = {'state_data': state_data,'state_name': state_name,'num_days':num_days,'start_date_str':start_date_str,'end_date_str':end_date_str, 'map_html': map_html}
#             return render(request, 'home2/state_details.html', context)
...
#                 # Create a list to store selected places for each day
#                 selected_places = []

#                 for i in range(num_days):
#                     # Select 1 or 2 places for each day
#                     day_places = state_data[i:i+2]
#                     selected_places.extend(day_places)

#                 state_data = selected_places
...
def display_places(request):
    if request.method == 'POST':
        # Get selected state, start date, end date, and selected places from the form data
        state_name = request.POST.get('state_name')
        start_date_str = request.POST.get('start_date')
        end_date_str = request.POST.get('end_date')
        num_people = request.POST.get('num_people')
        ...
        # Pass the map HTML to the context dictionary and render the template
            ...
            return render(request, 'home2/select_places.html', context)
        ...
        return render(request, 'home2/select_places.html')
...
def distance(lat1, lon1, lat2, lon2):
    ...
    return distance
    # Calculate the distance between the starting point and each place on the map
    distances = np.zeros(len(coordinates))
    for i in range(len(coordinates)):
        distances[i] = distance(start_lat, start_lon, coordinates[i][1], coordinates[i][0])

    # Find the shortest path between the starting point and each place using the Bellman-Ford algorithm
    shortest_paths = []
    for i in range(len(coordinates)):
        if i == 0:
            shortest_paths.append([i])
        else:
            path_lengths = [sum([distances[k] for k in path]) for path in shortest_paths]
            shortest_path = shortest_paths[np.argmin(path_lengths)]
            if len(shortest_path) == num_days:
                continue
            if i not in shortest_path:
                shortest_paths.append(shortest_path + [i])

    # Get the coordinates of the places in the shortest path
    shortest_path_coordinates = [coordinates[i] for i in shortest_paths[np.argmin([sum([distances[k] for k in path]) for path in shortest_paths])]]
    # Add markers to the map for each coordinate, and color the markers on the shortest path red
    for place in state_data:
        coordinate = (place['long'], place['lat'])
        place_name = place['place']
        color = 'blue'
        if coordinate in shortest_path_coordinates:
            color = 'red'
        folium.Marker(location=coordinate, popup=place_name, icon=folium.Icon(color=color)).add_to(my_map)

import heapq
from collections import defaultdict

def state_detail1(request):
    if request.method == 'POST':
        state_name = request.POST.get('state_name')
        start_date_str = request.POST.get('start_date')
        end_date_str = request.POST.get('end_date')
        ...
        if state_name in datavalue:
            ...
            # Create graph of recommended places
            graph = defaultdict(list)
            for i, place in enumerate(state_data):
                for j, other_place in enumerate(state_data):
                    if i != j:
                        distance = calc_distance(place['long'], place['lat'], other_place['long'], other_place['lat'])
                        graph[place['place']].append((other_place['place'], distance))
            
            # Find shortest path from starting location to each recommended place
            start_location = get_start_location()
            distances, paths = dijkstra(graph, start_location)
            recommended_places = [place for place in state_data if place['place'] in paths]
            recommended_places = sorted(recommended_places, key=lambda x: paths[x['place']])

            # Filter places based on number of days
            if num_days:
                recommended_places = [place for place in recommended_places if 'type' in place and place['type'] != 'None']
                recommended_places = recommended_places[:int(num_days)]

            # Create a folium map object
            my_map = folium.Map(location=[38.8949549, -77.0366456], zoom_start=10)

            # Add markers to the map for each recommended place
...
def get_start_location(request):
    if request.method == 'POST':
        start_location = request.POST.get('start_location')
        if start_location in datavalue:
            return start_location
        else:
            error_msg = 'Sorry, we do not have any information for the location you entered.'
            context = {'error_msg': error_msg}
            return render(request, 'home2/state_details.html', context)
    else:
        return render(request, 'home2/start_location.html')
import heapq
...
def state_detail1_val_generate(request):
    if request.method == 'POST':
        state_name = request.POST.get('state_name')
        start_date_str = request.POST.get('start_date')
        end_date_str = request.POST.get('end_date')
        ...
        if state_name in datavalue:
            ...
            num_clusters = 2  # Set the number of clusters as desired
            nbrs = NearestNeighbors(n_neighbors=num_clusters+1, algorithm='ball_tree').fit(coordinates)
            distances, indices = nbrs.kneighbors(coordinates)

            # Create a list of lines between the clusters
            lines = []
            for cluster_indices in indices:
                cluster_coordinates = [coordinates[i] for i in cluster_indices]
                lines.append(folium.PolyLine(locations=cluster_coordinates, weight=2, color='blue'))
...
def save_review(request):
    if request.method == 'POST':
        ...
        user_review = UserReview(state_name = state_name ,review_text=review_text, rating=rating)
        user_review.save()
